...
- [ ] As a shopper, open `/cart`. Confirm the "Delivering to" field is a **dropdown**, not a free-text box — options are grouped by zone name, listing that zone's cities.
- [ ] Before picking anything, confirm the delivery-fee estimate reflects the **default zone's** rate (no city chosen yet falls back to default, same as an unmatched city would server-side).
- [ ] Select a city that belongs to a real non-default zone (e.g. one of the Pokhara cities) — confirm the delivery-fee estimate updates **immediately** to that zone's rate (a selection saves right away, no debounce/typing settle time).
- [ ] Go to `/checkout` — confirm the city field there is the **same dropdown**, pre-filled from whatever city you selected on the cart page, and the COD handling fee note / order total reflect the zone matching that city. Change it to a different zone's city and confirm the total updates to match.
- [ ] Since you can no longer type an unmatched city through the UI, confirm the no-match fallback still works as a backend safety net: after saving a cart with a valid city, delete that city from its zone in admin (or edit the zone to drop it), then reload `/cart` — the field should still show the now-orphaned city as the selected option (not silently blank), and the fee shown should have fallen back to the default zone's rate.
...
- [ ] Cart page's "Delivering to" field always agrees with the order summary's "Delivery to {city}" line — it used to be possible for the field to show blank on load while the summary correctly showed a previously-saved city (stale local input state); the field is now a dropdown that reads the cart directly, so the two can't drift apart.
