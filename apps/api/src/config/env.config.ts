...
import { isOnReservedTenantSubdomain } from "#lib/admin-url.utils.js";
...
const envSchema = z
  ...
  .refine(
    ({ ADMIN_URL, TENANT_BASE_DOMAIN }) =>
      !isOnReservedTenantSubdomain(ADMIN_URL, TENANT_BASE_DOMAIN),
    {
      path: ["ADMIN_URL"],
      message:
        "ADMIN_URL must not sit on a reserved subdomain of TENANT_BASE_DOMAIN; use the bare base domain",
    },
  );
