...
**`GET /users/search` is an all-users typeahead (`name`/`handle` case-insensitive `contains`, capped at 10, followers-first), distinct from `../creators`' creator-only `search_creators()`.** `../gamification`'s Manual Actions (hand-award a badge, grant/dock XP) can target _any_ user, not just creators — a shopper or brand owner too — so the creator-scoped search isn't enough, and it's cheap: the `users_name_trgm_idx`/`users_handle_trgm_idx` GIN trigram indexes back the `ILIKE '%…%'` Prisma generates. It returns only `id`/`name`/`handle`/`avatarUrl` — never `email`/`phone` — since the admin UI just needs to pick a row and submit its id. Route order in `user.routes.ts` puts `/search` before `/:id` so the literal path wins over the uuid param.
