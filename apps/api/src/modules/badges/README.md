...
- `badge.controller.ts` / `badge.routes.ts` — `GET /badges/collection`, `PATCH /badges/:badgeId/display`, `PUT /badges/featured`. All `requireAuth`-only, scoped to the caller's own collection — there's no "view someone else's full collection" endpoint yet, only the public-profile featured subset (`../creators`).
...
**Featuring is a full-replace, not an incremental add/remove — `PUT`, not `PATCH`.** `updateFeatured(userId, badgeIds)` clears every currently-featured badge for the user and re-sets exactly the given ordered list in one transaction, matching how a drag-reorder UI naturally submits ("here's my new featured lineup") rather than a sequence of individual position edits that could race against each other.
