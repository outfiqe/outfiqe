...
**`Brand.tagReviewPolicy` and `Brand.autoApproveVerifiedBuyers` are read by `creator-looks` (tag-review status resolution) but not yet writable.** `PATCH /brands/me` does not accept them — a brand can't change its policy until the `tag-reviews` module lands (that chunk owns the brand-facing settings write). Every existing brand was defaulted to `TRUSTED_ONLY` / verified-buyer auto-approve on. See `../creator-looks/README.md` "Brand tag review".
