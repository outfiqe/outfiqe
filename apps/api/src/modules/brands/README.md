...
**`Brand.tagReviewPolicy` and `Brand.autoApproveVerifiedBuyers` are set through `PATCH /brands/me`** (both optional in `updateBrandProfileSchema`, flowing straight through `brandRepository.update`). They're read by `../creator-looks` when it resolves a new tag's review status, and by `../tag-reviews`. A policy change is **not retroactive** — it only affects tags submitted afterward; existing pending tags stay pending. Every brand was defaulted to `TRUSTED_ONLY` / verified-buyer auto-approve on. See `../creator-looks/README.md` "Brand tag review" and `../tag-reviews/README.md`.

**`GET /:id/products` now takes `optionalAuth` and threads the viewer through to `productService.listPublicByBrand`**, so a signed-in shopper sees real `isSaved` on a brand's product listing instead of it always reading `false` — see `../products/README.md`'s `isSaved` section for the batched, N+1-safe way that's computed.
