...
Read-only aggregate reporting and cross-entity search for a CRM tenant. Two report endpoints
(pipeline value/outcomes by stage, support-ticket health) and one global search over the tenant's
partners, customers, deals and tickets. Everything is computed in the database — SQL `GROUP BY` /
`FILTER` aggregates and `ILIKE` matching — never by fetching rows and reducing in application code.

## Structure

- `crm-reporting.constants.ts` — search query bounds (`MIN_SEARCH_QUERY_LENGTH`,
  `MAX_SEARCH_QUERY_LENGTH`), `SEARCH_RESULTS_PER_TYPE` (per-entity result cap), the
  ...
  window/max.
- `crm-reporting.types.ts` — `PipelineReport`, `TicketReport`, `CrmSearchResults` and their row
  shapes. Search partner/customer groups reuse `PartnerSummary` / `CustomerSummary` from
  `crm-relationships`.
- `crm-reporting.repository.ts` — the raw SQL. `pipelineStageBreakdown` is one `LEFT JOIN` of
  ...
  time (cast `::float8`, `NULL` when nothing is resolved). `searchDeals` / `searchTickets` are
  `title ILIKE '%q%'` scoped by `organization_id`, ordered by `updated_at`, capped.
- `crm-reporting.service.ts` — `getPipelineReport` folds the stage rows into branch totals;
  `getTicketReport` passes the repository shape straight through; `search` runs the four entity
...
**Technical:** `apps/admin` `CrmSearchBox` / `ReportsSection` → `reportingApi` (`apiClient`) →
`GET /api/crm/search`, `GET /api/crm/reports/pipeline`, `GET /api/crm/reports/tickets` →
`resolveTenant` → `requireAuth` → `requireAdvancedCrmFeatures` → permission middleware →
`crm-reporting.controller` → `.service` → `.repository` (`$queryRaw`) → Postgres.
...
- **The two report endpoints are gated on `reports:read`; search is not.** Reports are a single
