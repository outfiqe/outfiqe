...
- **Creating a second organization** (`POST /api/crm/organizations`) is deliberately not
  ...
  it's gated on `requirePlatformRole("platform:organizations:manage")` instead (see
  `platform-access/README.md`), registered before `crmAccessRoutes.use(resolveTenant, ...)` so it
  never runs through tenant resolution at all. `GET /organizations`/`/organizations/suggest` stay
  on the coarser `requirePlatformAccess` view gate — tenant creation is the sensitive action here,
  not looking at the list. The
...
- **`listOrganizations` is cursor-paginated** (`cursor`/`limit`, same `take: limit + 1` /
  `hasMore` shape every other paginated list in this codebase uses), rather than a single unbounded
  `findMany` — every tenant that signs up grows this table without bound. The admin Organizations
  screen uses `useInfiniteQuery` with a "Load more" button, the same pattern `AuditPage` uses.
