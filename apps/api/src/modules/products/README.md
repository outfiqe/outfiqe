...
- `products_search_vector_sync` (`BEFORE INSERT OR UPDATE ON products`, no column filter) computes the row's own vector via `products_compute_search_vector(id, name, brand_id, product_type_id)` — which now joins `product_types` for the type's `label` — and assigns it to `NEW.search_vector` directly. (Migration `20260903193000_convert_product_type_to_table` swapped the old `p_type text` arg for `p_product_type_id uuid` when garment types became a table.)
- A brand rename, category rename, or garment-type rename doesn't touch any row in `products` at all, so it can't fire that trigger on its own. `brands_cascade_search_vector` / `categories_cascade_search_vector` / `product_types_cascade_search_vector` (`AFTER UPDATE OF name`/`label`) and `category_links_cascade_search_vector` (`AFTER INSERT OR DELETE` on `_CategoryToProduct`) each cascade by running `UPDATE products SET search_vector = search_vector WHERE ...` — the value on the right is a no-op, but writing to that column is enough to fire the unconditional `BEFORE UPDATE` trigger above, which then overwrites it with a freshly computed one. This is why `products_search_vector_sync` deliberately has no `OF name, brand_id, product_type_id` column filter: an unfiltered `BEFORE` trigger is what makes the cascade trick work at all.
...
**Why this survives future `prisma migrate dev` runs:** the trigger functions and triggers themselves are pure hand-written SQL that Prisma's diff engine has no model of at all, so it will never generate a migration that touches or reverts them. The indexes are a different story, and got this wrong once (see incident note below): `products.name`, `users.handle`, `users.name`, and `creator_looks.caption` are ordinary `String` columns fully declared in `schema.prisma`, so Prisma fully manages them — an index that exists in the database but isn't declared via `@@index` reads to Prisma's diff engine as unwanted drift, and it will propose a `DROP INDEX` migration for it on the next `migrate dev`, even for a hand-written index created purely via raw SQL in an earlier migration. The fix is to declare the index in `schema.prisma` too, with `map` pinned to the exact name the hand-written migration already gave it so Prisma treats it as already satisfied rather than proposing a rename:
...
This also works for `searchVector` even though its column type (`Unsupported("tsvector")?`) isn't Prisma-managed — the index declaration and the column declaration are validated independently. The one rule this depends on: never edit `searchVector`'s type/attributes, or any of these `@@index` lines, in `schema.prisma` casually — any real change there needs a matching hand-written migration, not an auto-generated one. Don't run `prisma db push` against this schema for the same reason (it reconciles more aggressively and isn't migration-history-aware).

**Incident, 2026-08-19:** before these `@@index` declarations existed, `prisma migrate dev` proposed exactly the `DROP INDEX` migration described above for all 5 hand-written indexes (`products_search_vector_idx`, `products_name_trgm_idx`, `users_handle_trgm_idx`, `users_name_trgm_idx`, `creator_looks_caption_trgm_idx`). Someone accepted the prompt, it applied and dropped all 5, and the generated migration folder was deleted from disk before being committed — leaving the dev DB with an orphaned `_prisma_migrations` row (`20260819074137`) pointing at a migration no longer on disk, plus a real drift (missing indexes) that `prisma migrate dev` then refused to proceed past. Fixed by re-running the original `CREATE INDEX` statements, deleting the orphaned migration-history row, and adding the `@@index` declarations above so the same prompt can't recur.
...
**A single unrelated long word can still slip past that threshold — `word_similarity()`'s "best-matching substring" semantics cut both ways.** Searching "formal" surfaced "Performance Track Jacket" (tagged `Streetwear`/`Athleisure`, nothing formal about it) because `word_similarity('formal', 'Performance Track Jacket')` = 0.43 — just over the 0.4 gate — since "formal" is almost entirely a contiguous substring of the single word "performance" (`perf-ORMA-nce` / `f-ORMA-l`). That score sits right next to the genuine "jaket"/"Soft Blazer Jacket" typo case (0.44), so raising the threshold to exclude one would also exclude the other — no single cutoff on the whole-name score separates them. What does separate them: comparing the query against `p.name`'s individual words with plain `similarity()` instead of the whole string. Diluted-whole-string dilution isn't a problem per-word (nothing else is in the comparison to dilute it), so a real match to one word scores high ("jaket" vs "jacket" = 0.44) while a coincidental substring inside a longer, unrelated word scores low ("formal" vs "performance" = 0.19, "cargo" vs "camel"/"coat" = 0.2/0.1). `prisma/migrations/20260818170000_fix_product_search_trigram_false_positive` keeps the original `<%` operator as the fast, GIN-indexed admission check (so index usage on `products_name_trgm_idx` is unchanged for the common case) and ANDs it with an `EXISTS` over `regexp_split_to_table(p.name, '\s+')` requiring the same 0.4 threshold against at least one whole word — the extra per-word check only ever runs against the already-small set of rows the indexed `<%` scan admits, not the full table.
...
## Migrations that recreate `search_products` must load `pg_trgm` first

`search_products` is created with `SET pg_trgm.word_similarity_threshold = ...`. On a managed Postgres (production is Supabase) a non-superuser may set that parameter on a function only once the `pg_trgm` library is loaded in the session; otherwise the migration fails with `permission denied to set parameter`. Earlier migrations got away with it because something before them in the same deploy had already called a `pg_trgm` function. `20260917154458_load_pg_trgm_before_search_function` exists for that reason: it calls `similarity()` once so the migration after it always finds the extension loaded. Any future migration that recreates a function with a `pg_trgm.*` setting should be preceded by the same call.
...
## `image` on public product responses — responsive variants alongside `imageUrl`

Every public product shape (`PublicProduct`, `PublicProductDetail`) carries `image: ResponsiveImage | null` next to the unchanged `imageUrl` string. `image.url` is always exactly `imageUrl` (the original upload), so nothing that reads `imageUrl` needs to change; `image.sources` adds per-format (`avif`/`webp`/`jpeg`) `srcSet` strings and `image.lqip` a base64 blur placeholder, both for consumers that can render a `<picture>` with an instant preview.

`image` is built by `toResponsiveImage` (`#lib/responsive-image.utils.js`) from the first gallery `ProductImage`'s optional `imageAsset` link to `ImageProcessingAsset`. The list queries load only `{ status, encodedVariants, lqip }` for the first image via `withFirstImageAsset` (`take: 1`); `findPublicById` loads it for the whole gallery via `withGalleryImageAssets`. A completed asset with unparseable `encodedVariants` degrades to the fallback-only shape rather than throwing on the catalog read path.

`POST`/`PATCH /products` accept an optional `imageAssetIds` array that lines up one-to-one with `imageUrls` (a `null` slot means "no processed asset for this image"; a Zod refine rejects a length mismatch). The service calls `imageProcessingService.assertAssetsOwnedBy` before the write so a brand can't attach another user's asset (404 if any id isn't theirs), and the repository sets each `ProductImage.imageAssetId` by position — on create, and on `update`, which replaces the whole image set. Web gets these ids from `POST /uploads/pipeline`. Until a create/update actually sends them, `imageAsset` is null and `image` is the fallback-only shape `{ url: imageUrl, lqip: null, sources: [] }` — identical information to `imageUrl` alone.

## `isSaved` on every public listing is batched per page, not per product
