...
`image` is built by `toResponsiveImage` (`#lib/responsive-image.utils.js`) from the first gallery `ProductImage`'s optional `imageAsset` link to `ImageProcessingAsset`. Until an upload path populates `ProductImage.imageAssetId` (a later step of the image-pipeline integration), `imageAsset` is always null and `image` is the fallback-only shape `{ url: imageUrl, lqip: null, sources: [] }` — identical information to `imageUrl` alone. The list queries load only `{ status, encodedVariants, lqip }` for the first image via `withFirstImageAsset` (`take: 1`); `findPublicById` loads it for the whole gallery via `withGalleryImageAssets`. A completed asset with unparseable `encodedVariants` degrades to the fallback-only shape rather than throwing on the catalog read path.
