...
- `achievement.repository.ts` — Prisma queries only: `findEligibleAchievements` (active, non-admin-award achievements the user hasn't already unlocked) and one query per metric (`countActiveLooks`, `sumLikesReceived`, `countCommentsMade`, `countPurchases`, `countSalesGenerated`), plus `createUserBadge`.
