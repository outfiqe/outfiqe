...
AS $$
  ...
  matched AS (
    ...
    WHERE p.status = 'APPROVED'
      ...
      AND (p_thrift IS NULL OR p.is_thrift = p_thrift)
      AND (
        NOT p.is_thrift
        OR EXISTS (SELECT 1 FROM "product_sizes" ps_thrift WHERE ps_thrift.product_id = p.id AND ps_thrift.stock > 0)
      )
  ),
  counted AS (
