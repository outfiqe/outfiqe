...
- `components/ExploreFeed.tsx` — the `/explore` page: filter tabs (For You / Following / a trending tag), infinite-scroll feed, sidebar, and live updates via socket. Tab/layout state lives in the URL but is highlighted optimistically via `usePendingSelection` — see "Tab and layout switches highlight optimistically" below. Supports two layouts — List renders `PostCard`; Grid (the default) renders the lighter `PostGridCard` in a `react-masonry-css` grid (`EXPLORE_GRID_BREAKPOINT_COLUMNS`) — see "Grid layout is masonry, not CSS grid" below. Its first page is server-rendered — see "First feed page is server-rendered" below.
...
- `utils/trendingRank.ts` — `buildTrendingRankByPostId`, the one place that turns a ranked-tab feed
  page into rank-1/2/3 badge assignments — see "Rank badges follow `isTrending`, not array position"
  below.
...
**`PostTagPill` shows the creator's height alongside the size they tagged, reusing the "real fits, real sizes" convention `product-detail`'s `SeenOnCreators.tsx` already established on a product's own page**, rather than inventing a new way to convey fit. Both are formatted with the same `formatHeight` util. The pill only grows a second line when there's something to show — a tag with no size (predates the required-size rule) and a creator who hides their height (`FeedCreator.heightCm` is already `null` by the time it reaches the client — see the api's `creator-looks` README) both fall back gracefully, down to the original single-line, name-only pill.

**Grid layout is masonry, not CSS grid.** Every post used to render at one fixed `4/5` aspect, so `ExploreFeed`'s Grid layout used a plain `grid grid-cols-2 xl:grid-cols-3` — uniform-height tiles have no packing to do, and CSS `columns`' `column-fill: balance` was tried and rejected earlier for exactly the reasons `SavedPostsGrid`/`search` avoid it too (see their own history): it reads top-of-column-1 to bottom-of-column-1 first (wrong order for a relevance/recency-ranked feed) and can leave a tall empty gap in a short feed since `break-inside-avoid` tiles can only cut on tile boundaries. Once creators could choose a `PostLayout` (`PORTRAIT`/`SQUARE`/`TALL`, `apps/api/src/modules/creator-looks/README.md`) that plain grid rationale inverted — a real feed of mixed-height tiles in a fixed-row grid leaves ragged gaps exactly like the `columns` problem it was chosen to avoid. `ExploreFeed`'s Grid layout now reuses the same `react-masonry-css` + `EXPLORE_GRID_BREAKPOINT_COLUMNS` pattern `search` already had proven for these same `PostGridCard` tiles. The one wrinkle: the "Recent & popular" divider used to be one `<Fragment>` inside a single grid (a `col-span-full` row at a computed index) — masonry distributes children into separate column `<div>`s, so a divider can't sit "at position N" inside one `<Masonry>` and still span full width. `ExploreFeed` instead splits `posts` at `fallbackBoundaryIndex` into two slices, each its own `<Masonry>` block, with the divider paragraph rendered between them — same visual/functional result, no `col-span-full` needed.

**A staff viewer (platform or tenant) sees like/comment/follow/report treated differently, not uniformly hidden.** The backend rejects all four for an `ADMIN` or `TENANT_STAFF` role (`assertCanEngage`, see `apps/api/src/modules/creator-looks/README.md`), but `usePostCardState` (the single source of `isAdmin`/`likeDisabledReason` for both `PostCard` and `PostDetailModal`) applies three distinct UI treatments rather than one: **like** stays visible but disabled, wrapped in a `Tooltip` explaining why — the same explanatory copy the backend 403 would otherwise show a confused admin as a bare toast — since liking is a low-stakes, easily-explained restriction where showing the reason costs nothing; **comment** and **follow/report** are hidden outright (`onCommentClick={undefined}` falls through to `PostActionsRow`'s existing read-only-count fallback; `PostCardHeader` drops its whole follow+report cluster via `!isAdmin`) rather than shown disabled, since a comment box or a follow button an admin can never use is just visual noise once the reason is already established by the like tooltip elsewhere on the same card. The "Creators to follow" sidebar rail goes further and hides itself as a whole widget for an admin viewer, since every row in it would otherwise be a dead end.
...
**Rank badges follow `isTrending`, not array position.** Trending and For You used to badge posts "#1/#2/#3" purely by where they landed in the feed array (`TRENDING_RANKS[index]`), for every post in the tab. Both tabs can legitimately contain posts that were never actually trend-scored — `for_you`'s discovery list is interleaved with recent posts from creators the viewer follows regardless of those posts' own score (`creator-looks/README.md`'s "Re-ranks the trending pool for discovery" section), and `trending` itself falls back to a plain recency/like-count sort when nothing has scored yet at all — so a brand-new, zero-engagement post could land at index 0 or 1 and get badged as genuinely trending. Every feed post now carries a real `isTrending` boolean from the API (`creator-looks/README.md`'s "`isTrending` on each feed post reflects real trend score" section), and `buildTrendingRankByPostId` (`utils/trendingRank.ts`) numbers only the posts where that flag is true, in the order they appear, capped at the three `TRENDING_RANKS` slots — an interleaved or legacy-fallback post is never counted, so it never receives a badge it didn't earn.

**For You shows a standing "gets more personalized" hint, not a conditional one — and it's dismissible, permanently.** `scorePersonalized`'s follow/engagement/hashtag boosts (`creator-looks/README.md`) are a genuine no-op multiplier for a viewer with no signal yet, so For You and Trending can look identical to a new or logged-out viewer — a real question users asked, not a bug. Telling them apart correctly (does _this_ viewer have any real signal yet) needs data this component doesn't have client-side without a new fetch, so the hint runs off `tab === EXPLORE_TAB.FOR_YOU` alone rather than trying to detect "no signal yet" — it stays eligible to show even once personalization is genuinely active, which is an acceptable trade for not adding a fetch just to decide whether to show one line of copy. A user who already knows this shouldn't have to see it forever, so `FormBanner` (`@outfiqe/design-system`) takes an optional `onDismiss` and renders a close (`X`, `lucide-react`) button when it's passed; `ExploreFeed` wires it to `isForYouHintDismissed`/`rememberForYouHintDismissed` (`utils/forYouHint.ts`), a guarded `localStorage` flag — same "read/write wrapped in try/catch, never throws in a blocked-storage environment" shape as the PWA install/push prompts (`../pwa/constants/installPrompt.ts`/`pushOptIn.ts`) — so the dismissal is remembered across reloads and tab switches, not just for the current mount. `ExploreFeed.forYouHint.integration.test.tsx` covers all three states (shown with a dismiss control, hidden immediately on dismiss with the flag persisted, never shown again on a later mount once dismissed) via the same MSW-mocked-feed harness `ExploreFeed.authTiming.integration.test.tsx` already established — `ExploreFeed.tsx` itself still has no broader test file (heavy hook surface — auth gate, socket, infinite query, router — see the "no test yet for `useRecordLookView`" note below for the standing pattern of documenting rather than silently skipping), but this specific piece of behavior is now covered rather than exempted.
...
**An admin on a locked tab is redirected to `?tab=trending`.** `ExploreFeed` already showed Trending in place of For You and Following for admin accounts, but left the URL alone, so the address bar and the tab disagreed. Once auth has resolved as admin, a `router.replace` rewrites the tab param and keeps the layout param.
