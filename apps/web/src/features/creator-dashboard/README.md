...
- `hooks/useUpdateCreatorProfile.ts`, `useTaggableProducts.ts`, `usePendingPhotos.ts` — profile update, product search for tagging, and local photo/crop state for `PostModal` (the create flow; `EditPostForm` manages its own photo state separately — see below).
