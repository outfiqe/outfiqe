...
Shared test harness for `apps/admin` — the pieces every test needs that aren't specific to one
...
`tsconfig.test.json`'s `paths`), e.g. `import { mswServer } from "@test/integration/msw/server"`.
