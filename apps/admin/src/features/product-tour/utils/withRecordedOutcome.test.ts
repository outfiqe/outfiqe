...
describe("withRecordedOutcome", () => {
  it("creates the list when nothing was loaded yet", () => {
    const progress = withRecordedOutcome(
      undefined,
      { tourKey: "crm-dashboard", version: 1, outcome: TOUR_OUTCOME.COMPLETED },
      RECORDED_AT,
    );
  ...
  it("replaces the earlier entry for the same tour and keeps other tours", () => {
    const progress = withRecordedOutcome(
      ...
      { tourKey: "crm-dashboard", version: 2, outcome: TOUR_OUTCOME.COMPLETED },
      RECORDED_AT,
    );

    expect(progress.tours).toHaveLength(2);
    expect(progress.tours).toContainEqual(expect.objectContaining({ tourKey: "brand-dashboard" }));
