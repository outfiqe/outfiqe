      ...
      <div key={index}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Input
            ...
            inputMode="decimal"
            aria-invalid={conditionValueErrorKey(index) in errors}
            aria-describedby={`${idPrefix}-value-error-${index}`}
            value={condition.value}
            onChange={(e) =>
              onChange(
                conditions.map((c, i) => (i === index ? { ...c, value: e.target.value } : c)),
              )
            }
            className="w-28"
          />
        ...
        <FieldErrorMessage
          id={`${idPrefix}-value-error-${index}`}
          message={errors[conditionValueErrorKey(index)]}
        />
      </div>
    ))}
    <FieldErrorMessage message={errors.conditions} />
    <Button
      type="button"
      variant="outline"
      size="sm"
